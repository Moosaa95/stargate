        ...
        <nav className={`py-4 lg:px-14 ${isSticky ? "sticky top-0 left-0 right-0 border-b bg-white duration-300" : ""}`}>
            <div className="flex justify-between items-center text-base gap-8">
                ...
                <ul className="md:flex space-x-12 hidden">
                    ...
                    {NAV_LINKS.map(({href, label, key}) => <Link   className="regular-16 text-gray-90 flexCenter cursor-pointer pb-1.5 transition-all hover:font-bold" href={href} key={key}>{label}</Link>)}
